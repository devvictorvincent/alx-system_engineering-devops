...
def recurse(subreddit, hot_list=[], after="", count=0):
    ...
    headers = {
        "User-Agent": "linux:0x16.api.advanced:v1.0.0 (by /u/victor_)"
    }
    ...
    # Sending a GET request to the subreddit's hot posts page
    ...
    if response.status_code == 404:
        return None
    # Parsing the JSON response and extract relevant data
    results = response.json().get("data")
    after = results.get("after")
    count += results.get("dist")

    # Appending post titles to the hot_list
